...
class TestExcelReportExporter(unittest.TestCase):
    def test_generar_archivo_excel_no_vacio_y_encabezados(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = exporter.exportar(output, "Cuadro de Calificación Anual", context, rows)
            self.assertTrue(Path(result).exists())
            self.assertGreater(Path(result).stat().st_size, 0)

            wb = load_workbook(result)
            ws = wb.active
            self.assertEqual(ws["A1"].value, "UEEH")
            self.assertEqual(ws["A2"].value, "CUADRO DE CALIFICACIÓN ANUAL")
            self.assertEqual(ws["A3"].value, "San Salvador de Cañaribamba")
            self.assertEqual(ws["A10"].value, "N°")
            self.assertEqual(ws["B10"].value, "Nómina")
            self.assertEqual(ws["B11"].alignment.horizontal, "left")
            wb.close()
