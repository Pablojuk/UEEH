...
class HtmlReportRenderer:
    def render(self, context: dict[str, Any], rows: list[dict[str, Any]]) -> str:
        ...
        values = {
            ...
            "logo_inst_src": self._path_to_file_uri(self._resolve_logo_path(context, "institucional")),
            "logo_mineduc_src": self._path_to_file_uri(self._resolve_logo_path(context, "ministerio")),
    ...
    @staticmethod
    def _resolve_logo_path(context: dict[str, Any], logo_type: str) -> Any:
        institucion = context.get("institucion", {}) if isinstance(context.get("institucion"), dict) else {}
        if logo_type == "institucional":
            return (
                context.get("logo_path")
                or context.get("logo_institucional_path")
                or institucion.get("logo_path")
                or institucion.get("logo_institucional_path")
            )
        return (
            context.get("logo_ministerio_path")
            or context.get("logo_mineduc_path")
            or institucion.get("logo_ministerio_path")
            or institucion.get("logo_mineduc_path")
        )
